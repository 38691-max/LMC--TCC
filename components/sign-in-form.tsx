...
export function SignInForm({
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, setError } = useForm<typeSignInSchema>({ resolver: zodResolver(signInSchema) })
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Login com conta do Google ou APPLE
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form noValidate onSubmit={handleSubmit(async ({ email, password }) => {
              ...
              async onError({ error }) {
                switch (error.message) {
                  ...
                  case "Email not verified":
                    setError("email", { message: "Este e-mail não foi verificado" });
                  case "Invalid email or password":
                    setError("password", { message: "Senha incorreta." });
                  default:
                    toast.error("Login Falou.", { duration: 5000 });
                    break;
                }
              },
              async onSuccess() {
                toast.success("login sucedido!!", { duration: 2000 })
                setTimeout(() => {
                  redirect("/dashboard")
                }, 2000)
              },
            ...
            <FieldGroup>
              ...
              <Field>
                <Button disabled={isSubmitting} type="submit">{isSubmitting ? <Spinner /> : "login"}</Button>
                <FieldDescription className="text-center">
                  Não tem nenhuma conta? <a href="/sign-up">Cadastro</a>
                </FieldDescription>
              </Field>
            </FieldGroup>
          </form>
      ...
      <FieldDescription className="px-6 text-center">
        Ao clicar em Login, você concorda com nossos <a href="/terms">Termos de serviço</a> e <a href="/politicy">Política de privacidade</a>.
      </FieldDescription>
